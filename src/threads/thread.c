...
void thread_yield(void)
{
  ...
  old_level = intr_disable();
  // 여기서 priority 비교하고 스레드 올리는거 하면 될듯?
...
void update_awake_first_time(void)
{
...
void check_and_wake_thread(int64_t ticks)
{
  if (!sleep_list_check())
  {
    while (get_awake_first_time() <= ticks)
    {
      if (!sleep_list_check())
      {
        wake_thread();
...
//   {
//     struct list_elem *first_entity = list_begin(&ready_list);
//     struct thread *first_thread = list_entry(first_entity, struct thread, elem);
...
schedule(void)
{
  ...
  // list_insert_ordered(&ready_list,&prev->elem,less,NULL);
}
